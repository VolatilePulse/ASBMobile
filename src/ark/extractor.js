...
export class Extractor {
   ...
   extract() {
      ...
      if (this.bred) {
         if (this.exactly)
            ...
            if (this.values[TORPOR] == Utils.RoundTo(tempStat.calculateValue(this.m[TORPOR], !this.wild, this.TE, imprintingBonusFromFood), Ark.Precision(TORPOR)))
      ...
      do {
         ...
         for (var i = 0; i < 7; i ++) {
            ...
            if (!this.results[i].checked && this.m[i].notUsed) {
               ...
               this.results[i] = [new Stat(-1, 0)];
               // We have to set speed too, unfortunately
               this.results[SPEED] = [new Stat(-1, this.results[SPEED][0].Ld)];
    ...
    * 
    * @param {[number, number]|any} indices - An array of index arrays to use on the results object
    * @param {boolean} [returnBool = false] - If set to true, will return a boolean value instead of an array 
    * 
    * @returns {(boolean|array)} - All matching stats that are required to keep the levels "true"
    */
